package com.ditchoom.boringssl.gradle

import org.gradle.api.Project
import java.net.URI

/**
 * Next-version derivation, cloned from the socket/buffer/webrtc convention: read the latest published
 ...
 *
 * Greenfield adaptation: until the first `com.ditchoom.boringssl:boringssl-jvm` release exists, the
 * metadata fetch fails; we fall back to `0.0.0` so the default patch bump yields the RFC's target
 * `0.0.1` (RFC §10 step 3). Once the first real release lands the fetch succeeds and normal increment
 * resumes with no code change.
 */
internal fun Project.computeNextVersion(
    umbrellaArtifact: String = "boringssl-jvm",
    snapshot: Boolean,
): String {
    val base =
        try {
            val xml =
                URI("https://repo1.maven.org/maven2/com/ditchoom/boringssl/$umbrellaArtifact/maven-metadata.xml")
                    .toURL()
                    .readText()
